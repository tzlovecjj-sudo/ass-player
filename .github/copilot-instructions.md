...
- 严格禁止在未取得明确授权的情况下，将本地更改直接提交并推送到远端仓库（包括创建分支后直接推送、在远端开启 PR、或通过 CI/自动化工具合并分支）。任何会影响远端仓库的操作，AI 代理必须先获得仓库维护者或指定负责人的书面明确许可（例如在对话中获得“同意我现在把这些修改 push 到远端”的明确回复）。
- 当需要进行代码修改时，AI 代理应该：
  - 在本地工作区中创建并保存更改（可以创建本地分支），但不要执行任何 push 或打开 PR。
  - 把变更摘要、影响范围、及示例补丁（可直接作为 patch）提交给用户审核。
  - 在得到用户的明确许可后，由用户或在得到明确授权的代理执行 push/PR 操作。
- 所有与 Git/GitHub 相关的消息（包括 commit message、PR 标题与说明、Issue 描述、Review 评论）必须使用中文（简体）。这条规则对自动生成的或由 AI 协助编写的内容同样适用。
- 如果用户要求 AI 代理“先提交但不推送”或“仅在本地保存提交”，应在提交信息里写明“本地提交，仅供审阅，禁止推送（Do not push）”。
- 若遇到紧急安全修复等确需立即推送的情况，AI 代理应在做任何推送前尽可能尝试联系仓库管理员，并在变更说明中清楚记录授权方与授权时间。

> 备注：本节为强制执行规则。AI 代理在任何自动化脚本或工作流（包括 CI/CD）中也应遵守相同的原则，不应在没有人类明确授权的情况下自动向远端写入或合并变更。

## 本地优先的协作工作流程（强制）

- 在你与 AI 协作期间，所有由 AI 代理或 AI 生成的修改默认仅保存在本地工作区，且不得在未经你明确书面授权的情况下向远端仓库提交或推送任何变更（包括通过 CI/自动化工具）。
- AI 代理在准备修改时应：
  - 在本地工作区中创建变更，并把变更摘要、影响范围、和可应用的补丁（patch）提交给你审阅；
  - 如果需要本地提交以便审阅或运行测试，AI 可以在本地创建 commit，但该 commit 的提交信息必须以中文撰写，并包含明确标记："本地提交，仅供审阅，禁止推送（Do not push）"；
  - 绝不在没有你书面同意（例如在对话中明确回复“同意我现在把这些修改 push 到远端”）的情况下执行 `git push`、在远端创建分支或打开 PR。
- 若你明确授权 AI 代理替你执行远端操作（push/PR），AI 代理应在推送或打开 PR 之前在对话中回显将要推送的分支名、提交摘要、PR 标题与说明，并在对话中记录授权者与授权时间。任何远端写操作都应在对话中留下审计记录（谁授权、何时授权、推送到哪个分支）。
- 本地优先工作流也适用于 CI/CD 与自动化脚本：这些工具不得在未经授权时触发对远端仓库的写操作；若确实需要自动化推送，必须先取得仓库管理员的明确授权并在变更说明中写明授权细节。

> 强调：这一节为强制流程，AI 代理在所有交互中必须遵守；如有例外，须先获得你（仓库维护者或指定负责人）的书面明确许可并在对话中记录该许可。

## 完整本地验证流程（必须遵守）

- 每次完成新需求的修改或 bug 修复后，必须严格按照本地优先流程执行以下步骤，且这些步骤由 AI 代理在本地完成并向你汇报结果：
  1. 在项目虚拟环境中运行所有单元测试与 E2E 测试（例如 `python -m pytest -q` 或 `python run_tests.py`），并把测试输出、失败用例与日志整理成摘要提交给你审阅；
  2. 在本地启动完整服务以进行人工验收测试：执行 `python start.py`（或团队约定的开发启动脚本），在真实浏览器中完成手动检查（包括全屏、播放、字幕渲染、交互等关键场景）；
  3. 将本地测试的结果、关键截图（如发现 UI 回归）与复现步骤整理成审阅报告并提交给你；仅在你确认人工验收通过并书面同意后，AI 代理或你本人方可把更改推送到远端并打开 PR。

- AI 代理在执行上述步骤时应保留可证明的操作记录（测试命令、完整输出、截图文件与时间戳），并在对话中附上简要审核摘要以便你做出是否推送的决定。

> 说明：该流程确保每次变更都有本地自动化测试与人工验收两层把关，降低回归风险，并把最终是否推送的权利完全交还给你（仓库维护者）。
